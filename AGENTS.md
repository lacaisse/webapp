...
- The app exposes an MCP endpoint at **`/api/mcp`** (Streamable HTTP, stateless JSON) so agents can operate the dashboard. Canonical URL: `https://<APP_DOMAIN>/api/mcp`. Handler plumbing in `services/mcp/` (`handler.ts` builds a per-request `McpServer` from `@modelcontextprotocol/sdk`; `tools.ts` registers the toolset).
...
- **Tenancy in tools**: MCP calls arrive on one endpoint with a bearer token, so the fund is a tool *parameter* (a domain string) — a deliberate deviation from the host-derived DAL. `services/mcp/authz.ts::requireFundAccessForUser(userId, fundDomain, minRole)` is the tenant gate: it resolves the canonical domain, checks `FundMember` and the role rank, and every tool must go through it before touching fund data. Role floors mirror the pages (members OPERATOR+, token tools ADMIN+).
